...
| 坐标校验 | validateActions() | Server-side 坐标钳制 + 最小尺寸保障 |
...
"画红色圆"
  ...
  ├── Canvas.jsx: 渲染红色圆形
  ├── canvasSummary.js: 更新画布摘要（含 zone 区域描述，供下一次指令使用）
  └── History.jsx: 添加历史记录
```
...
| 值 | 含义 | 触发条件 | 前端行为 |
|----|------|---------|---------|
| `success` | 成功解析 | 指令明确且可完整执行 | 执行 actions |
| `optimized` | 优化执行 | 指令模糊（如"小圆"→设 radius:40） | 执行 actions |
| `error` | 无法理解 | ASR 严重错误 / 指令超出能力 | 不执行，提示重说 |
...
| 画布摘要 | 后续指令参考已绘制图形位置 | canvasSummary.js（含 zone 区域） | ✅ 已实现 |
...
=======
| 坐标网格系统 | 3×3 区域划分精确定位 | 系统提示词 + validateActions() 后处理 | ✅ 已实现 |
| 尺寸分级 | 小/中/大三级量化约定 | 系统提示词约定 | ✅ 已实现 |
| 碰撞规避 | 新图形与已有保持 50px+ 间距 | 系统提示词规则 | ✅ 已实现 |
| SVG 质量约束 | SVG 必须含有效绘图元素，不可为空 | 系统提示词规则 + 后处理 | ✅ 已实现 |
>>>>>>> origin/main
...
| SVG 质量取决于 LLM | DeepSeek V4 Flash SVG 评分 88.9% | 复杂物体可能不完美；已增加 SVG 质量规则约束 |
...
## 11. 绘画质量改进（2026-06-14）

### 11.1 背景

四模型基准测试结论：**四模型分类成绩完全相同（95.5%），失败用例也一致——精度瓶颈不在模型，在系统提示词的表达方式。**

原有系统提示词规则 6"坐标在画布中央区域（400,300 附近）"实际上有害——导致所有图形挤在画布中心，与"左上角""右边"等空间描述自相矛盾。
...
**新规则 6-9（`server/proxy.js`）：**
```
6. 画布坐标系统（800×600，3×3 网格）：
   ┌────────────┬────────────┬────────────┐
   │ 左上角      │ 中上方      │ 右上角      │
   ...
   └────────────┴────────────┴────────────┘
   位置关系映射；"左边"→x<267, "右边"→x>533, 等

7. 尺寸约定：小(30-50) / 中(60-100) / 大(120-180)
8. 避免重叠：新图形与已有保持 50px+ 间距
9. SVG 质量要求：必须有有效绘图元素，不可为空
```

### 11.3 后端后处理校验

`server/proxy.js` 新增 `validateActions()` 函数，在 LLM 返回后自动：
- 坐标钳制到画布范围（x:10-790, y:10-590）
- 最小可见尺寸（radius≥20, width/height≥20）

### 11.4 画布摘要增强

`src/services/canvasSummary.js` 新增 `getZone()` 函数，为每个图形计算所在画布区域（如"左上""中中""右下"），追加 `zone` 字段供 LLM 做空间决策。

### 11.5 验证结果

| 场景 | 旧版结果 | 新版结果 |
|------|---------|---------|
| "左上角" | x=100, y=100 | **x=133, y=100**（精确匹配网格） |
| "圆在左边/矩形在右边" | x=200 / x=600 | **x=133 / x=607**（更精准） |
| "小圆" | radius=30 | **radius=40**（中值稳定） |
| "大矩形在右下角" | 无对应 | **x=567,y=425,w=200,h=150** |
| "在旁边画"（有上下文） | 无碰撞规避 | **间距 170px > 50px** |
| SVG 有效性 | 88.9% | 全部有效 |
>>>>>>> origin/main
